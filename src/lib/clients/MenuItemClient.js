...
class MenuItemClient {
  ...
  async getRestaurants() {
    ...
    let auxrestaurants = [];
    let restaurants = [];
    let newRestaurants = await restaurantClient.getAll(options);
    newRestaurants.map(restaurant => restaurants.push(restaurant))
    auxrestaurants.push(newRestaurants);
    while (newRestaurants.length !== 0) {
      options.next();
      newRestaurants = await restaurantClient.getAll(options);
      newRestaurants.map(restaurant => restaurants.push(restaurant))
    }
    return restaurants;
  }
  ...
  async getAll(menuItem, restaurant = null) {
    ...
    if (typeof restaurant === 'string') {
      ...
      let res = restaurants.find(res => res.integrationCode === restaurant);
      console.log(res.integrationCode)
      if (res) {
